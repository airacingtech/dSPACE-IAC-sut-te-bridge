...
#include <SensorUtilities/Cuda/DsHostDeviceMacro.h>
...
namespace PPRadarDetectionsBase
{
	...
	struct FComplexFloat3
	{
		FComplexFloat X,Y,Z;

		DS_HOSTDEVICE inline FComplexFloat3 operator*(const float floatVar) const
		{
	...
	struct FDetection
	{
		...
		float ElevationAngle = 0.f; //[rad] bElevation angle of detection relative to sensor. Positive Z coordinate will produce positive angle value. Affected by accuracy and resolution in config.
		...
		FComplexFloat3 RawEField{}; //Complex raw E-Field of the detection. Influenced by sending power, distance and reflection characteristics of target.
		...
		friend std::ostream &operator<<(std::ostream &o, const FDetection &)
		{
			o << "NYI";
			return o;
		}

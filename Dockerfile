...
RUN source /opt/ros/iron/setup.bash && \
    ...
    echo 'source /root/ros_ws_aux/install/setup.bash' >> /root/.bashrc
...
RUN mkdir -p /root/record_log && \
    ...
    source /root/ros_ws_aux/install/setup.bash && \
    ...
    echo 'source /root/ros2_bridge_ws/install/setup.bash' >> /root/.bashrc

ENTRYPOINT ["sh", "-c", ". /opt/ros/iron/setup.bash && . /root/ros_ws_aux/install/setup.bash && . /root/ros2_bridge_ws/install/setup.bash && ros2 run sut_te_bridge SutTeBridgeNode --ros-args -p use_sim_time:=$SIM_CLOCK_MODE"]
...
ENTRYPOINT ["sh", "-c", ". /opt/ros/iron/setup.bash && . /root/ros_ws_aux/install/setup.bash && ros2 launch foxglove_bridge foxglove_bridge_launch.xml"]
